...
for UBUNTU_VERSION in "${!UBUNTU_VERSION_MAP[@]}"; do
    ...
    for ARCH in "${ARCHS[@]}"; do
        ...
        sudo debootstrap --arch=$ARCH --foreign --components=main,universe,multiverse --variant=minbase --include=apt,dbus,dbus-x11,wget,curl,vim,net-tools,lsb-release,locales,tzdata,passwd $CODE_NAME $ROOTFS_DIR $REPO_URL
...
EOF
        ...
        echo "Adding missing groups to /etc/group in the rootfs..."
...
EOF
        ...
        # Step 10: Packaging the rootfs into a tarball (excluding the root folder)
        echo "Packaging the rootfs into a tar.gz archive..."  # Changed from .tar.xz to .tar.gz
        pushd $ROOTFS_DIR > /dev/null
        sudo tar --numeric-owner -czf $TARBALL_NAME .  # Change from -cJf (xz) to -czf (gzip)
        popd > /dev/null
